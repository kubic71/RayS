...
def shark_top1_decision(labels, scores):
    shark_label_set = ["Shark", "Fin", "Water", "Fish", "Carcharhiniformes", "Lamnidae", "Lamniformes"]
    return not label_set_match(shark_label_set, labels[:1])

for query_limit in [25, 50, 100, 200, 400, 800, 1600]:
    DIR = f"output/shark_attack_top1"
    pathlib.Path(DIR).mkdir(parents=True, exist_ok=True)

    model = GVisionModel(decision_fn=shark_top1_decision)
    ...
    exp_name = f"shark_queries={query_limit}_dist={dist}"
